...
static void ethernetif_input(const void *argument);
static void RMII_Thread(const void *argument);
...
/* don't set NETIF_FLAG_ETHARP if this device is not an ethernet one */
    ...
    if (HAL_GetREVID() == 0x1000) {
        /*
          This thread will keep resetting the RMII interface until good frames
          are received
        */
        osThreadDef(RMII_Watchdog, RMII_Thread, osPriorityRealtime, 0,
...
void HAL_ETH_MspInit(ETH_HandleTypeDef *ethHandle) {
    ...
    if (ethHandle->Instance == ETH) {
        ...
        */
        ...
        GPIO_InitStruct.Pin       = ETH_TX_EN_Pin | ETH_TXD0_Pin;
        GPIO_InitStruct.Mode      = GPIO_MODE_AF_PP;
        GPIO_InitStruct.Pull      = GPIO_NOPULL;
        GPIO_InitStruct.Speed     = GPIO_SPEED_FREQ_VERY_HIGH;
        GPIO_InitStruct.Alternate = GPIO_AF11_ETH;
...
void HAL_ETH_MspDeInit(ETH_HandleTypeDef *ethHandle) {
    if (ethHandle->Instance == ETH) {
        ...
        */
        ...
        HAL_GPIO_DeInit(GPIOG, ETH_TX_EN_Pin | ETH_TXD0_Pin);
...
void RMII_Thread(const void *argument) {
    (void) argument;
